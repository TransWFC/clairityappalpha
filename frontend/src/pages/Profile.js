...
const Profile = () => {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                const response = await fetch('http://10.0.130.21:5000/api/auth/me', {
    ...
    const handleSendVerificationCode = async () => {
        try {
            const response = await fetch('http://10.0.130.21:5000/api/auth/send-verification-code-user', {
                method: 'POST',
                headers: {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response = await fetch('http://10.0.130.21:5000/api/auth/update', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
